...
def calculate_targets_difference(output_image, target_mask, out, mode="median",):
    """
...
def plot_all_outputs(input_dir, out, fig_px_size=None, fig_dpi=100):
    ...
    for filename in os.listdir(input_dir):
        ...
        full_dict[model_name][stim_name] = plt.imread(os.path.join(input_dir, filename))
    ...
    for i, (model_name, model_outputs) in enumerate(full_dict.items()): # i = row index
        for j, (stim_name, output_image) in enumerate(model_outputs.items()): # j = col index
            index = i*cols + j + 1
...
def create_RHS_table(input_dir, out, normalized=True):
    ...
    for model_name, outputs in full_dict.items():
        normalization = 1
        if normalized:
            normalization_key = tuple(key for key in full_dict[model_name] if "we_thick" in key.lower())
            normalization = abs(calculate_RHS_table_values(full_dict[model_name][normalization_key[0]])) if len(normalization_key) else 1
        for stim_name, means in outputs.items():
            if stim_name not in table_values:
                table_values[stim_name] = {}
            table_values[stim_name][model_name] = calculate_RHS_table_values(full_dict[model_name][stim_name], normalization)
